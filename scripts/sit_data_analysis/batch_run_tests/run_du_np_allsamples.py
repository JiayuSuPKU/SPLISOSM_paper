### Run the non-parametric differential usage tests on one mouse olfactory bulb (MOB) and two coronal brain section (CBS) samples
import os
import torch
import numpy as np
import pandas as pd
...
def _prepare_visium_rbp_anndata(data_dir, sample_id):
    # load the count data
    adata_visium = sc.read_h5ad(f"{data_dir}/anndata/{sample_id}_spatial.h5ad") # short-read gene counts
    ...
    # subset to RBP genes
    adata_visium.var['is_rbp'] = adata_visium.var['features'].isin(
...
if __name__ == '__main__':
    ...
    for _dir in [res_dir, f"{res_dir}/anndata", f"{res_dir}/du_np_results"]:
    ...
    for _sample_id in ['mob', 'cbs1', 'cbs2']:
        ...
        ## (1) prepare RBP expression data and run spatial variability tests
        # load visium expression data for RBP genes
        adata_visium_rbp = _prepare_visium_rbp_anndata(f'{data_dir}/{_sample_id}', _sample_id)
        # reorder rbp adata spot order to match ONT adata
        adata_visium_rbp = adata_visium_rbp[adata_ont_sv.obs.index, :].copy()

        # run spatial variability tests on RBP expression
        df_sv_hsic = _run_hsic_visium(adata_visium_rbp)
        df_sv_sparkx = _run_sparkx_visium(adata_visium_rbp)

        # compare the results
        print(f"Number of significant genes (HSIC): {(df_sv_hsic['pvalue_adj'] <= 0.01).sum()}")
        print(f"Number of significant genes (SPARK-X): {(df_sv_sparkx['pvalue_adj'] <= 0.01).sum()}")
        print(
            f"Number of overlapping significant genes: {
            df_sv_hsic[df_sv_hsic['pvalue_adj'] <= 0.01].index.intersection(
                df_sv_sparkx[df_sv_sparkx['pvalue_adj'] <= 0.01].index
            ).shape[0]}"
        )

        # save SV results
        adata_visium_rbp.var['pvalue_adj_hsic'] = df_sv_hsic['pvalue_adj']
        adata_visium_rbp.var['pvalue_adj_sparkx'] = df_sv_sparkx['pvalue_adj']
        adata_visium_rbp.var['is_visium_sve'] = adata_visium_rbp.var['pvalue_adj_sparkx'] < 0.01

        adata_visium_rbp.write_h5ad(f"{res_dir}/anndata/{_sample_id}_visium_rbp_{date}.h5ad")
        ...
        # save results
        df_rbp_pval.to_csv(f"{res_dir}/du_np_results/{_sample_id}_rbp_du_np_combined_{date}.csv", index=False)
