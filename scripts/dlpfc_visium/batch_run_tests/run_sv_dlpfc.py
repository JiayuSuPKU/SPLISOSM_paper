### Run spatial variability tests on transcriptome 3'end diversity (TREND) events for all 12 DLPFC samples
import os
import torch
import numpy as np
import pandas as pd
...
def _prepare_anndata(sample_id, data_dir):
    ...
    adata_visium = sc.read_visium(
        f"{data_dir}/spaceranger_out/{sample_id}/", 
    ...
    sierra_out_dir = f'{data_dir}/sierra_peaks_default/{sample_id}/counts_default'
    ...
    # keep only peak detected in more than one sample aka "Merged"
    merged_peaks = pd.read_table(f"{data_dir}/sierra_peaks_default/peak_default.merged.txt")
    ...
    adata_peak = adata_peak[adata_peak.obs_names.isin(adata_visium.obs.index), :]
